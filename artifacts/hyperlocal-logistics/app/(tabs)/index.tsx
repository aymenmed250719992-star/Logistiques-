...
export default function HomeScreen() {
  ...
  const isCourier = user?.role === "courier" || !user;
  const greeting = user?.name?.split(" ")[0] ?? (isCourier ? "Courier" : "Sender");
  const subtitle = isCourier ? "Ready to deliver today?" : "Manage your deliveries";
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        <View style={{ flex: 1 }}>
          <View style={styles.nameRow}>
            ...
            <View style={[styles.roleBadge, { backgroundColor: isCourier ? colors.primary + "20" : colors.accent + "20" }]}>
              ...
              />
              <Text style={[styles.roleBadgeText, { color: isCourier ? colors.primary : colors.accent }]}>
      ...
      >
        {isCourier ? <CourierDashboard /> : <SenderDashboard />}
      </ScrollView>
    </View>
  );
}
...
function createStyles(colors: ReturnType<typeof useColors>) {
  return StyleSheet.create({
    ...
    notifBtn: { width: 42, height: 42, borderRadius: 21, alignItems: "center", justifyContent: "center", borderWidth: 1 },
